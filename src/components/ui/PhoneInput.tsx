...
export default function PhoneInput({
  ...
  function renderName(country: Country) {
    if (!search) return (
      <span className="text-[#2D2926] font-medium">
        {country.name} <span className="text-[#2D2926]/40 font-normal">({country.dialCode})</span>
      </span>
    )
    ...
    return (
      <span className="text-[#2D2926]">
        ...
        {' '}<span className="text-[#2D2926]/40 font-normal">({country.dialCode})</span>
      </span>
    )
  }
      ...
      <div className="max-h-64 overflow-y-auto">
        {filtered.length === 0 ? (
          ...
          filtered.map(country => {
            const isSelected = country.dialCode === countryCode && country.code === selected.code
            return (
              <button
                key={country.code}
                type="button"
                onMouseDown={e => e.preventDefault()}
                onClick={() => { onCountryCodeChange(country.dialCode); setOpen(false); setSearch('') }}
  ...
  return (
    <div className="relative" ref={containerRef}>
      {/* Unified input bar */}
      <div className={`flex items-stretch rounded-xl border transition-all bg-white overflow-hidden ${ringClass}`}>
        {/* Country selector trigger */}
        <button
          type="button"
          onClick={handleOpen}
          className="flex items-center gap-1.5 pl-3 pr-2.5 border-r border-[#e8dcc4] hover:bg-[#F5EDE0]/60 transition-colors flex-shrink-0"
        >
          <span className="text-lg leading-none">{selected.flag}</span>
          <ChevronDown
            className={`w-3 h-3 text-[#2D2926]/40 transition-transform duration-150 ${open ? 'rotate-180' : ''}`}
          />
        </button>

        {/* Dial code */}
        <div className="flex items-center pl-2.5 pr-1.5 text-sm font-semibold text-[#2D2926]/60 select-none flex-shrink-0">
          {selected.dialCode}
        </div>
        ...
        <input
          ...
          className="flex-1 py-3 pr-3 text-sm text-[#2D2926] placeholder:text-[#2D2926]/30 focus:outline-none bg-transparent min-w-0"
        />
      </div>
