...
export async function getEmailConfig(): Promise<EmailConfig> {
  ...
  const from      = db['email_from']           || process.env.EMAIL_FROM     || 'Forestry <info@theforestry.me>'
  const adminEmail = db['admin_email']         || process.env.ADMIN_EMAIL    || 'info@theforestry.me'
